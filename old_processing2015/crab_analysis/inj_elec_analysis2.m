function inj_elec_analysis2(Nerve_number, EIT_number, Pair_number, Inj, y1, y2)
%
...
load(sprintf('mat/N%03i_EIT%03i.mat',Nerve_number,EIT_number))

Fs = 16384;
T = 1e3*((1:Fs/2)/Fs-.15);

hasTrig = find(any(abs(TrigData(T>-2 & T<2,:))>1e4));
...
pairChanges = [0,find(diff(hasTrig)>1)];
...
sub_demod(Xinj,Fc,tstr)
subplot(3,1,2), ylim(y1)
subplot(3,1,3), ylim(y2)
saveas(gcf,sprintf('N%03i_%04iHz_EIT%03i_Pair%i_V%i-V%i.png',...
